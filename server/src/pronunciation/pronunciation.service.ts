...
export class PronunciationService {
    constructor(
        @InjectModel('en')
        private readonly enPronunciation: Model<PronunciationDocument>,
        @InjectModel('fr')
        private readonly frPronunciation: Model<PronunciationDocument>,
        @InjectModel('es')
        private readonly esPronunciation: Model<PronunciationDocument>,
    ...
    async result(voice: Express.Multer.File, info: CreatePhrases) {
        const deepgram = new Deepgram(
            this.configService.get('DEEPGRAM_ACCESS_TOKEN')
        );
